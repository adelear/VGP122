...
Sprite::Sprite(std::string goID, std::string f, int x, int y, int w, int h, int r, int c, int offsetX, int offsetY, double a, SDL_RendererFlip flip) : filename(f), position(x, y), w(w), h(h), angle(a), rows(r), cols(c), offsetX(offsetX), offsetY(offsetY), flipFlag(flip)
{
